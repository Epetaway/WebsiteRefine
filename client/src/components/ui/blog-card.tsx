...
export default function BlogCard({
  ...
  return (
    <article
      ...
      className={[
        "group relative overflow-hidden rounded-2xl bg-card transition-colors border border-border",
        "hover:border-dominant focus-within:border-dominant",
      ].join(" ")}
      data-testid={`blog-card-${post.id}`}
    >
      <Link
        to={href}
        className="block outline-none focus-visible:ring-2 focus-visible:ring-dominant focus-visible:ring-offset-2"
      ...
      >
        ...
        <div className={isDefault ? "p-5 sm:p-6" : "p-4"}>
          <div className="mb-2 flex flex-wrap items-center gap-2 text-[11px] uppercase tracking-wide">
            <span className="rounded-full bg-dominant/20 px-2 py-0.5 text-dominant">{post.category}</span>
            <span className="text-textSecondary">•</span>
            <time className="text-textSecondary" dateTime={post.publishedAt}>
              {dateLabel}
            </time>
            <span className="text-textSecondary">•</span>
            <span className="text-textSecondary">{post.readTime} min</span>
          </div>
          ...
          <p className={["mt-2 text-sm text-textSecondary", isDefault ? "line-clamp-3" : "line-clamp-2"].join(" ")}>
            {post.excerpt}
          </p>

          {isDefault && post.takeaway ? (
            <div className="mt-3 text-sm">
              <span className="font-semibold text-textPrimary">Takeaway: </span>
              <span className="text-textSecondary">{post.takeaway}</span>
            </div>
          ) : null}

          <div className="mt-3 inline-flex items-center gap-1 text-sm font-medium text-textPrimary">
            Read more
            <svg
              width="16"
              height="16"
