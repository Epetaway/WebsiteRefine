...
export default function Header() {
  ...
  return (
    <header className="w-full bg-gray-50">
      ...
      <div className="mx-auto max-w-[1040px] px-4">
        <div className="flex items-center justify-between py-4">
          ...
          <nav className="hidden md:flex items-center gap-4">
            {nav.map((item) => (
              <NavLink
                ...
                to={item.to}
                className={({ isActive }) =>
                  `px-4 py-1 text-sm font-medium -tracking-[0.04em] transition-colors ${
          ...
          <div className="hidden md:flex items-center gap-2">
            <Link
              to="/about"
              className="px-4 py-2 text-sm font-normal -tracking-[0.02em] text-gray-900 bg-gradient-to-b from-gray-200 to-gray-200 rounded-lg shadow-[0_1px_0_0_rgba(99,106,126,0.6)] hover:opacity-90 transition-opacity"
            >
              Get In Touch
            </Link>
            <a
              ...
              className="px-4 py-2 text-sm font-normal -tracking-[0.02em] text-white bg-gradient-to-b from-purple-400 to-purple-600 rounded-lg shadow-[0_1px_0_0_rgba(99,106,126,0.6)] hover:opacity-90 transition-opacity"
            >
              Get Resume
            </a>
          </div>
          ...
          {/* Mobile Menu Button */}
          <button
            className="md:hidden inline-flex items-center justify-center rounded-lg px-3 py-2 text-sm bg-gray-200"
            onClick={() => setOpen((v) => !v)}
            aria-expanded={open}
      ...
      {/* Mobile Navigation */}
      {open && (
        <div id="mobile-nav" className="md:hidden bg-white shadow-sm border-t border-gray-200">
          <div className="mx-auto max-w-[1040px] px-4 py-3 flex flex-col gap-2">
            ...
            <Link
              to="/about"
              onClick={() => setOpen(false)}
              className="rounded-lg px-3 py-2 text-sm font-semibold text-gray-900 bg-gray-200"
            >
              Get In Touch
            </Link>
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="rounded-lg px-3 py-2 text-sm font-semibold text-white bg-gradient-to-b from-purple-400 to-purple-600"
              onClick={() => setOpen(false)}
            >
              Get Resume
            </a>
